...
def fetch_alpha_vantage_daily_adjusted(
	...
	"""调用 Alpha Vantage 的 TIME_SERIES_DAILY_ADJUSTED 接口。"""
	params = {
	...
	for attempt in range(1, max_retries + 1):
		rate_limiter.wait()
		try:
			resp = session.get(ALPHAVANTAGE_API_URL, params=params, timeout=30)
			i = resp.status_code
			if i != 200:
...
def parse_daily_adjusted_to_dataframe(symbol: str, payload: dict) -> pd.DataFrame:
	"""解析 TIME_SERIES_DAILY_ADJUSTED 的 JSON 为 DataFrame。"""
...
def main(argv: Optional[List[str]] = None) -> None:
	parser = argparse.ArgumentParser(
		description="合规抓取股票日频(复权)数据，来源为 Alpha Vantage 官方开放接口"
	)
	parser.add_argument(
		"--symbols",
		"-s",
		nargs="+",
		required=True,
		help="股票代码列表，如 AAPL MSFT 600519.SHH 等"
	)
	parser.add_argument(
		"--api-key",
		"-k",
	...
	parser.add_argument(
		"--output",
		"-o",
		default=os.path.join("data", f"stocks_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"),
		help="输出 CSV 路径，默认 data/stocks_时间戳.csv"
	)
	parser.add_argument(
		"--outputsize",
		choices=["compact", "full"],
		default="full",
		help="compact 约100条近期数据，full 为全量"
	)
	parser.add_argument(
		"--calls-per-minute",
		type=int,
	...
	for symbol in args.symbols:
		...
		payload = fetch_alpha_vantage_daily_adjusted(
			symbol=symbol,
			api_key=api_key,
		...
		df = parse_daily_adjusted_to_dataframe(symbol, payload)
		if df.empty:
